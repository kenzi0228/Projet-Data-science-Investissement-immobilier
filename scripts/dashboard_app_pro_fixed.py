...
class DashboardAppPro:
    ...
    def _clip_q(series, qlow=0.01, qhigh=0.99):
        """
        Coupe les extrêmes d'une série sur les quantiles [qlow, qhigh].
        Retourne une série alignée (même index filtré).
        """
    ...
    def _build_sidebar(self, parent):
        ...
        self.w_charges = self._add_slider(scrollable, "Charges (%)", 0, 40, 25)
        ...
        self.w_outliers = self._add_checkbox(scrollable, "Supprimer outliers (IQR x2)")
    ...
    def _add_range_slider(self, parent, label, min_v, max_v, default):
        """Double slider (min/max) correctement indenté"""
    ...
    def _compute_yields(self, d: pd.DataFrame) -> pd.DataFrame:
        """Calcule rendement brut/net en fonction des sliders actuels"""
    ...
    def _load_data(self):
        ...
        try:
            ...
            # CHEMIN À ADAPTER À TON PROJET SI BESOIN
    ...
    def _apply_filters(self):
        ...
        try:
            ...
            # Calcul rendement (selon sliders actuels)
            ...
            # Trim quantiles final pour stabilité des graphes
            ...
            self.count_label.config(
                text=f"📊 {n_final:,} / {n_init:,} ({(n_final/n_init*100 if n_init else 0):.1f}%)".replace(",", " ")
            )
    ...
    def _render_overview(self, parent):
        """Vue d'ensemble avec KPIs + histogramme prix/m² (trim 1–99%)"""
        ...
        ax.set_xlabel('Prix/m² (€)', color=self.colors['fg'])
        ...
        if 'prix_m2' in d.columns:
            ...
            if len(p) > 0:
                ax.axvline(p.median(), color=self.colors['danger'], linestyle='--', linewidth=2,
    ...
    def _render_prix(self, parent):
        """Analyse des prix (scatter + histogramme trim 1–99%)"""
        for w in parent.winfo_children():
            w.destroy()
        ...
        if 'rendement_net' in ds.columns:
            ...
            fig.colorbar(scatter, ax=ax1, label='Rendement net (%)')
        else:
            ax1.scatter(ds['surface_reelle_bati'], ds['prix_m2'],
                        s=30, alpha=0.6, c="#cccccc",
        ...
        # Histogramme prix (trim 1–99%)
        ax2 = fig.add_subplot(122)
        ax2.set_facecolor(self.colors['card'])
        
        if 'prix_m2' in d.columns:
            p = self._clip_q(d['prix_m2'], 0.01, 0.99)
            ax2.hist(p, bins=40, edgecolor='white')
            if len(p) > 0:
                ax2.axvline(p.median(), color=self.colors['danger'],
    ...
    def _render_carte(self, parent):
        """Carte départements (barres comparatives)"""
        for w in parent.winfo_children():
            w.destroy()
        
        d = self.current_filtered_data
        
        # Vérifier code_departement
        ...
        if 'rendement_net' in d.columns:
            rdt_med = grp['rendement_net'].median()
        else:
            rdt_med = None
        
        dept_stats = pd.DataFrame({'prix_med': prix_med})
        if rdt_med is not None:
            dept_stats['rdt_med'] = rdt_med
        dept_stats['nb'] = grp['prix_m2'].count()
...
{'Biens atteignant objectif rendement : ' + f'{len(objectif_ok):,} ({pct_objectif:.1f}%)' if rdt_med is not None else ''}
        """.replace(",", " ")
        ...
        if pd.notna(prix_total_med) and prix_total_med <= PERSONA['budget_max']:
            evaluations.append(("💰 ACCESSIBLE",
                f"Prix médian {prix_total_med/1000:.0f}k€ dans votre budget",
                self.colors['success']))
        else:
            evaluations.append(("💸 TENDU",
                f"Prix médian {0 if pd.isna(prix_total_med) else prix_total_med/1000:.0f}k€ > budget {PERSONA['budget_max']/1000:.0f}k€",
                self.colors['warning']))
        
        if pd.notna(surf_med) and PERSONA['surface_min'] <= surf_med <= PERSONA['surface_max']:
        ...
        recommandations = d
        if 'valeur_fonciere' in recommandations.columns and 'surface_reelle_bati' in recommandations.columns:
            recommandations = recommandations[
                (recommandations['valeur_fonciere'] <= PERSONA['budget_max']) &
                (recommandations['surface_reelle_bati'].between(PERSONA['surface_min'], PERSONA['surface_max']))
            ]
        else:
            recommandations = d.head(0)
        
        if 'rendement_net' in recommandations.columns and not recommandations.empty:
            recommandations = recommandations[
